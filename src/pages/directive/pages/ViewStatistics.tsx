...
const ViewStatistics = () => {
  ...
  const [disableDownloadStatistics, setDisableDownloadStatistics] =
    useState(false);
  const userInfo = useSelector((state: any) => state.userInfo);
  const directives =
    userInfo.profile === "directiva"
      ? [userInfo.directive.name]
      : userInfo.profile === "sostenedor"
      ? userInfo.directives
      : [];
  const [directivesOp, setDirectivesOp] = useState<any>(
    userInfo.profile === "directiva"
      ? []
      : userInfo.profile === "sostenedor"
      ? [{ value: "Mostrar Todo", label: "Mostrar Todo" }].concat(
          userInfo.directives.map((v: string) => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      if (userInfo.profile === "virgo") {
        ...
        let key = "value";
        const unique: any = [
          ...new Map(schools.map((item: any) => [item[key], item])).values(),
        ];
        setDirectivesOp(
          [{ value: "Mostrar Todo", label: "Mostrar Todo" }].concat(unique)
        );
      }
      const row_ = [];
      ...
      for (let i = 0; i < cols.length; i++) {
        ...
        if (element === "coursesNames") {
          ($.title = <div style={{ display: "flex", gap: 20 }}>{element}</div>),
        ...
        if (element === "inProgress") {
          ($.title = <div style={{ display: "flex", gap: 20 }}>{element}</div>),
            ($.render = (i_: any, { inProgress }: any) => {
              return (
                <>
                  <div
                    style={{ display: "flex", flexDirection: "column", gap: 5 }}
                  >
                    {inProgress.map((ip: any) => {
                      return (
                        <Tag color={"green"} key={ip._id}>
                          {ip?.course?.name}
                        </Tag>
                      );
                    })}
  ...
  const generateStatisticsFile = async () => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].inProgress.length > 0) {
        for (let j = 0; j < data[i].inProgress.length; j++) {
          const clone_ = JSON.parse(JSON.stringify(clone));
          const name = data[i].inProgress[j];
          console.log("name: ", name)
          clone_.inProgress = name.course.name;
          toDownload.push(clone_);
        }
      } else {
        toDownload.push(clone);
  ...
  return (
    ...
    >
      ...
      <Table
        style={{ lineBreak: "auto", whiteSpace: "normal" }}
        columns={columns}
